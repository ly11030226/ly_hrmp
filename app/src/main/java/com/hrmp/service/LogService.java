...
import com.hrmp.util.SecurityUtils;
...
public class LogService extends Service {
    ...
    private static final boolean collectEncryptLogOrNot = false; //false是不收集加密日志,true是收集加密日志
    ...
    private static final int SDCARD_LOG_FILE_SAVE_DAYS =3;							//sd卡中日志文件的最多保存天数 7天---修改为保存3天（16-9-2）
    ...
    class LogCollectorThread extends Thread {
        ...
        public LogCollectorThread(){
            ...
            LogUtils.d(TAG, "LogCollectorThread is create");
        }
    ...
    private void clearLogCache() {
        ...
        try {
            ...
            outputGobbler.start();
            if (proc.waitFor() != 0) {
                LogUtils.e(TAG, " clearLogCache proc.waitFor() != 0");
                recordLogServiceLog("clearLogCache clearLogCache proc.waitFor() != 0");
            }
        } catch (Exception e) {
            LogUtils.e(TAG, "clearLogCache failed", e);
    ...
    private List<String> getAllProcess() {
        ...
        try {
            ...
            if (proc.waitFor() != 0) {
                LogUtils.e(TAG, "getAllProcess proc.waitFor() != 0");
                recordLogServiceLog("getAllProcess proc.waitFor() != 0");
            }
        } catch (Exception e) {
            LogUtils.e(TAG, "getAllProcess failed", e);
...
//		commandList.add("*:D");
        ...
        try {
            ...
            LogUtils.e(TAG, "CollectorThread == >" + e.getMessage(), e);
            recordLogServiceLog("CollectorThread == >" + e.getMessage());
        }
    }

    private void collectEncryptLog() {
        String mPid = android.os.Process.myPid()+"";
        String cmds = "logcat *:v | grep \"(" + mPid + ")\"";
        Process mProcess = null;
        BufferedReader mReader = null;
        FileWriter fileWriter = null;
        try {
            fileWriter = new FileWriter(getLogPath());
        }catch (IOException e) {
            LogUtils.i(TAG,"Excetion="+e.getMessage()+","+e.getStackTrace());
        }
        try {
            ...
            while((line = mReader.readLine()) != null){
                ...
                if(fileWriter != null){
                    String content = (sdf.format(new Date())+" "+line+"\r\n");
                    fileWriter.write(SecurityUtils.encryptOrDencrypt(content,100));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public String getLogPath(){
        ...
        if(CURR_LOG_TYPE == MEMORY_TYPE){
            ...
            LogUtils.d(TAG, "Log stored in memory, the path is:"+LOG_PATH_MEMORY_DIR + File.separator + logFileName);
            return LOG_PATH_MEMORY_DIR + File.separator + logFileName;
        }else{
            CURR_INSTALL_LOG_NAME = null;
            LogUtils.d(TAG, "Log stored in SDcard, the path is:"+LOG_PATH_SDCARD_DIR + File.separator + logFileName);
    ...
    private void deployLogSizeMonitorTask() {
        ...
        LogUtils.d(TAG, "deployLogSizeMonitorTask() succ !");
    ...
    private void cancelLogSizeMonitorTask() {
        ...
        LogUtils.d(TAG, "canelLogSizeMonitorTask() succ");
    }
    ...
    private void checkLogSize(){
        if(CURR_INSTALL_LOG_NAME != null && !"".equals(CURR_INSTALL_LOG_NAME)){
            ...
            LogUtils.d(TAG, "checkLog() ==> The size of the log is too big?");
            if(file.length() >= MEMORY_LOG_FILE_MAX_SIZE){
                LogUtils.d(TAG, "The log's size is too big!");
                new LogCollectorThread().start();
            }
        }
    }
    ...
    private void createLogDir() {
        LogUtils.d(TAG, "createLogDir="+LOG_PATH_SDCARD_DIR);
//		File file = new File(LOG_PATH_MEMORY_DIR);
        ...
        FileTools.dir_exist(FileTools.errorLogDir);
    }
    ...
    private void deleteSDcardExpiredLog() {
        ...
        if (file.isDirectory()) {
            ...
            for (File logFile : allFiles) {
                ...
                if (fileName.contains(FileTools.errorLogDirName))
                {
                    continue;
                }
                else
                {
                    ...
                    if (canDeleteSDLog(createDateInfo)) {
                        logFile.delete();
                        LogUtils.d(TAG, "delete expired log success,the log path is:"
...
//				Log.d(TAG, "delete expired log success,the log path is:"+_file.getAbsolutePath());
    ...
    private String getFileNameWithoutExtension(String fileName){
        ...
        LogUtils.d(TAG, "getFileNameWithoutExtension() filename="+fileName);
        return fileName;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(Intent.ACTION_MEDIA_UNMOUNTED.equals(intent.getAction())){	//存储卡被卸载
                if(CURR_LOG_TYPE == SDCARD_TYPE){
                    LogUtils.d(TAG, "SDcar is UNMOUNTED");
                    CURR_LOG_TYPE = MEMORY_TYPE;
                    new LogCollectorThread().start();
                }
            }else{															//存储卡被挂载
                if(CURR_LOG_TYPE == MEMORY_TYPE){
                    LogUtils.d(TAG, "SDcar is MOUNTED");
    ...
    /**
     * 删除com.chinamobile.uc/pgm 下PGM 的log日志
     */
    public void clearPgmLog(){
        try{
            String dirPGMFile = Environment.getExternalStorageDirectory().getPath()+"/com.hrmp/pgm";
            File file_file = new File(dirPGMFile);
            if (file_file.exists()) {
                ...
                if(files != null){
                    for (int i = 0; i < files.length; i++) {
                        ...
                        String createDateInfo = getFileNameWithoutExtension(files[i].getName());
                        if (canDeleteSDPGMLog(createDateInfo)) {
                            files[i].delete();
                        }
                    }
                    LogUtils.d(TAG, "clearPgmProvisonFile----删除成功");
                }
            }
        }catch (Exception e){
            e.printStackTrace();
            LogUtils.d(TAG, "clearPgmProvisonFile----删除失败");
        }
    }
    /**
     * 判断sdcard上的日志文件是否可以删除
     ...
     * @return
     */
    public static boolean canDeleteSDPGMLog(String createDateStr) {
        boolean canDel = false;
        SimpleDateFormat sdfYMD = new SimpleDateFormat("yyyy-MM-dd");//日志名称格式
        Calendar calendar = Calendar.getInstance();
        calendar.add(Calendar.DAY_OF_MONTH, -1 * SDCARD_LOG_FILE_SAVE_DAYS);//删除7天之前日志
        Date expiredDate = calendar.getTime();
        try {
            Date createDate = sdfYMD.parse(createDateStr);
            canDel = createDate.before(expiredDate);
        } catch (ParseException e) {
            LogUtils.e(TAG, e.getMessage(), e);
            canDel = false;
